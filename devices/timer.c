...
timer_sleep (int64_t ticks) 
{
    ...
    // push thread to the slept_list
    // block thread.
    // set intr level ->old level
...
timer_interrupt (struct intr_frame *args UNUSED)
{
  ...
  thread_tick ();
    if(thread_mlfqs)
   mlfqs_calculation(thread_current());

	while(!list_empty(&sleep_list)){
      tempElem = list_front(&sleep_list);
	  tempThread = list_entry (tempElem, struct thread, elem);

	  	if(tempThread->wake_up_tick > ticks )
	  		break;
        else {
	  	list_remove (tempElem);
	  	thread_unblock(tempThread);
        //sema_up(&tempThread->sleep_sema);
        }
	}
}
